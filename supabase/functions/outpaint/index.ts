...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!checkRateLimit(authResult.userId, 'outpaint', 10, 60)) {
    ...
    const { data: job } = await supabase.from('outpainting_jobs').select('*').eq('id', job_id).single();
    if (!job) {
    ...
    await supabase.from('outpainting_jobs').update({ status: 'processing' }).eq('id', job_id).eq('status', 'pending');

    const replicateToken = Deno.env.get('REPLICATE_API_TOKEN');
    if (!replicateToken) {
      await supabase.from('outpainting_jobs').update({ status: 'failed', error_message: 'API not configured', completed_at: new Date().toISOString() }).eq('id', job_id);
    ...
    try {
      ...
      await supabase.from('outpainting_jobs').update({
        status: 'completed',
      ...
      await supabase.from('outpainting_jobs').update({
        status: 'failed',
        error_message: (apiErr as Error).message || 'Outpainting API unavailable',
        completed_at: new Date().toISOString(),
      }).eq('id', job_id);
    }
