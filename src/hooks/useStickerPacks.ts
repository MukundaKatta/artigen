...
import { createPack, getMyPacks, getPublicPacks, getSavedPacks, savePack as savePackApi, unsavePack as unsavePackApi } from '@/services/sticker-pack.service';
...
export function useStickerPacks(userId?: string) {
  ...
  const fetchPublicPacks = useCallback(async (page: number = 0) => {
    setLoading(true);
    const { data } = await getPublicPacks(page);
  ...
  const fetchSavedPacks = useCallback(async () => {
    if (!userId) return;
    const { data } = await getSavedPacks(userId);
  ...
  const create = useCallback(
    async (name: string, description?: string) => {
      if (!userId) return;
      const { data, error } = await createPack(userId, name, description);
      if (data) {
  ...
  const savePack = useCallback(
    async (packId: string) => {
      ...
      const { error } = await savePackApi(userId, packId);
      if (error) {
        // Revert on error
  ...
  const unsavePack = useCallback(
    async (packId: string) => {
      ...
      setSavedPacks((prev) => prev.filter((p) => p.id !== packId));
      const { error } = await unsavePackApi(userId, packId);
      if (error && removed) {
        // Revert on error
