...
/** Any table name known to the generated Database type. */
...
export async function toggleRelation(
  ...
  try {
    if (shouldExist) {
      // Join-table rows are simple { col: value } maps; the generated Insert
      // type can't be expressed generically here, so narrow the payload only.
      const { error } = await supabase
        .from(table)
        .insert(matchColumns as never);
      if (error) {
